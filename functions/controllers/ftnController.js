...
// de-duplicated array of allowed values (empty array if nothing matches).
...
/* ---------------- Controller ---------------- */
exports.enrichGeneralContracting = async (req, res) => {
    const source = { ...(req.query || {}), ...(req.body || {}) };
    const minutesBack = parseInt(source.minutesBack || 60, 10);
    const limit = parseInt(source.limit || 50, 10);
    ...
    try {
        ...
        if (rows.length === 0) {
            return res.json({ status: 'ok', batch: 0, leads: 0, nonLeads: 0, errors: 0 });
        }
        ...
        return res.json({ status: 'ok', batch: rows.length, leads, nonLeads, errors });
    } catch (err) {
        console.error('[ftn] enrichGeneralContracting error:', err);
