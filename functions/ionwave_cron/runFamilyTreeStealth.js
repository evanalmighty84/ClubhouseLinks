...
async function loadProxyLines() {
    ...
    // env list variable
    if (process.env.PROXY_LIST) return process.env.PROXY_LIST.split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
    // file
    try {
        const txt = await fsp.readFile(PROXY_FILE, 'utf8');
        return txt.split(/\r?\n/).map(s=>s.trim()).filter(Boolean);
    } catch (e) {
        return [];
    }
...
async function probeExitIp(proxy) {
    // small HTTP probe with axios using proxy option (http/s) or socks agent fallback
    try {
        if (proxy.proto && proxy.proto.startsWith('socks')) {
            // attempt socks-proxy-agent if installed
            let SP;
            try { SP = require('socks-proxy-agent'); } catch (e) { throw new Error('socks proxy requested but socks-proxy-agent not installed'); }
            const agentUrl = `${proxy.proto}://${proxy.username ? proxy.username + ':' + proxy.password + '@' : ''}${proxy.host}:${proxy.port}`;
            const AgentCtor = SP.SocksProxyAgent || SP;
            const agent = new AgentCtor(agentUrl);
            const r = await axios.get('https://ipinfo.io/json', { httpsAgent: agent, httpAgent: agent, timeout: 12000 });
