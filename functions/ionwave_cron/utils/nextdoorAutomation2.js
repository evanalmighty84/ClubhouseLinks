...
async function saveMessagedPost(post) {
    const { url, author, location } = post;
    try {
        console.log(`💾 Saving to DB: URL=${url}, Author=${author}, Location=${location}`);
        await pool.query(
            'INSERT INTO nextdoor_messages (post_url, author, location, city) VALUES ($1, $2, $3, $4) ON CONFLICT (post_url) DO NOTHING',
            [url, author, location, CITY]
        );
    } catch (err) {
