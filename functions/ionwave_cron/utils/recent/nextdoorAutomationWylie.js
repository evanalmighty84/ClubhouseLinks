...
const runNextdoorAutomation = async () => {
    ...
    const launchOpts = {
        ...
        args: ['--disable-blink-features=AutomationControlled', ...(headless ? ['--no-sandbox', '--disable-dev-shm-usage'] : [])],
    };
        ...
        for (const { label, query, type, needsMostRecent } of SEARCH_TERMS) {
            ...
            for (const [i, lead] of newLeads.entries()) {
                ...
                if (canDM) {
                  ...
                  console.log(`🚫 DM cap reached — will enrich & save without sending.`);
                }
                */

                // Always enrich (no DM dependency)
                let phone = null,
                    email = null,
