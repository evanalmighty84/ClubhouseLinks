...
async function saveMessagedPost(post, didSendMessage = false) {
    const { url, author, location, city = 'The Colony', leadType } = post;

    try {
        console.log(`💾 Saving to DB: URL=${url}, Author=${author}, Location=${location}, LeadType=${leadType}, MessageSent=${didSendMessage}`);

        if (didSendMessage) {
            await pool.query(
                `INSERT INTO nextdoor_messages
                 (post_url, author, location, city, lead_type, message_sent_at, message_sent)
        ...
        for (const { label, query, type, needsMostRecent } of SEARCH_TERMS) {
            ...
            for (const [i, lead] of newLeads.entries()) {
                ...
                try {
                    ...
                    lead.leadType = type;
                    await saveMessagedPost(lead, status === 'dm_sent');
                } catch (err) {
