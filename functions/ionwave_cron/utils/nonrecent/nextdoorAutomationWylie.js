...
const SEARCH_TERMS = [
    ...
    { label: 'Handyman Plumber', query: 'handyman plumber', type: 'handyman'}
];
...
async function saveMessagedPost(post) {
    const { url, author, location, city = 'Wylie', leadType } = post;
...
const runNextdoorAutomation = async () => {
    ...
    try {
        ...
        await page.fill('input[data-testid="email-address-input"]', process.env.NEXTDOOR_USERNAME4);
        await page.fill('input[data-testid="password-input"]', process.env.NEXTDOOR_PASSWORD4);
        await Promise.all([
